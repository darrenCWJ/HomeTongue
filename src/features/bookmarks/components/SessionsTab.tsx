...
export function SessionsTab({
  ...
  return filteredSessions.length === 0 && !isTourMode ? (
    <div className="flex flex-col items-center justify-center py-20 text-center px-6">
      ...
      <h3 className="text-lg font-medium text-foreground mb-1">No saved sessions</h3>
    ...
    >
      <button className="absolute top-2 right-2 p-1.5 rounded-full text-faint hover:text-muted-foreground hover:bg-muted transition-colors">
        <MoreHorizontal size={16} />
      </button>
      <div className="p-5 flex items-center gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <p className="font-semibold text-foreground text-base truncate">Morning Greeting</p>
          </div>
          ...
          <p className="text-xs text-faint truncate mt-0.5 italic">你好，好高興認識你！</p>
        </div>
      ...
      {/* Expanded preview */}
      <div className="border-t border-border-subtle p-3 space-y-2 bg-background">
        <div className="flex items-end gap-2 justify-start">
          ...
          <div className="max-w-[75%] rounded-2xl rounded-bl-sm bg-card border border-border px-3 py-2">
            <p className="text-sm font-semibold leading-snug text-foreground">你好，好高興認識你！</p>
